...
<script type="text/javascript">
    layui.use(['laydate', 'laypage', 'layer', 'table','form'], function(){
        ...
        table.render({
            ...
            ,height: 495
            ,url: '/express-project/payEdit/selAll'     //数据接口
            ,title: '快递类型维护'
            ,totalRow: true
           ,limit:10
            ,cols: [[ //表头
                {field: 'payid', title: '序号', width:90}
                ,{field: 'paytype', title: '支付方式', width:90}
                ,{field: 'paynode', title: '备注', width:90}
                ,{field: 'paytime', title: '创建时间', width:100,totalRow: true}
        ...
        table.on('tool(test)', function(obj){
            ...
            if(obj.event === 'del'){
                layer.confirm('真的要删除吗？', function (index) {
                    $.post("/express-project/payEdit/delPay",{'payid':data.payid},function(result)
                            {
    ...
    $('#add').on('click',function(){
        layui.use('layer', function() {
            ...
            layer.open({
                ...
                area: ['840px', '480px'], //宽高
                content: "/express-project/payEdit/add",
                end: function () {
                    location.reload();
                }
            });
