...
<script type="text/javascript">
    layui.use(['laydate', 'laypage', 'layer', 'table','form'], function(){
        ...
        table.render({
            ...
            ,height: 495
            ,url: '/express-project/itemEdit/selAll'     //数据接口
            ,title: '物品类型维护'
            ,totalRow: true
           ,limit:10
            ,cols: [[ //表头
                {field: 'itemid', title: '序号', width:90}
                ,{field: 'itemtype', title: '物品类型', width:90}
                ,{field: 'itemtime', title: '创建时间', width:100,totalRow: true}
        ...
        table.on('tool(test)', function(obj){
            ...
            if(obj.event === 'del'){
                layer.confirm('真的要删除吗？', function (index) {
                    $.post("/express-project/itemEdit/delItem",{'itemid':data.itemid},function(result)
                            {
    ...
    $('#add').on('click',function(){
        layui.use('layer', function() {
            ...
            layer.open({
                ...
                area: ['840px', '480px'], //宽高
                content: "/express-project/itemEdit/add",
                end: function () {
                    location.reload();
                }
            });
